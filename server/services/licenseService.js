  ...
  try {
    ...
    if (user.license_instance_id && user.license_instance_id.license_code) {
      ...
      if (isExpired) {
        ...
        if (
          user.pending_license?.license_code &&
          user.pending_license.is_downgrade
        ) {
        ...
        const graceDays =
          licenseDef?.grace_period_days !== undefined
            ? licenseDef.grace_period_days
            : 5;
    ...
    if (entity_type === "COMPANY") {
      ...
      if (!subscription) {
        return {
          ...
          expiry_date: null,
          message:
            "Company license pool is empty. Purchase licenses to assign to users.",
        };
      }
  ...
  try {
    ...
    if (typeof entityOrUserId === "object" && entityOrUserId.entity_type) {
      ...
      if (entity.entity_type === "USER") {
        ...
        console.warn(
          "⚠️ [LICENSE SERVICE] COMPANY entity type used for feature access. This should use USER entity.",
        );
    ...
    return {
      hasAccess: true,
      reason: subscription.is_expired
        ? "PLAN_FEATURE_EXPIRED"
        : "ACCESS_GRANTED",
  ...
  try {
    ...
    if (typeof entityOrUserId === "object" && entityOrUserId.entity_type) {
      if (entityOrUserId.entity_type === "USER") {
        ...
        console.warn(
          "⚠️ [LICENSE SERVICE] COMPANY entity type is not valid for limit checks. Limits are per-user.",
        );
        return {
          canConsume: false,
    ...
    if (currentUsage >= limit) {
      const limitLabel =
        limitType === "TOTAL" ? "" : ` for this ${limitType.toLowerCase()}`;
      return {
        canConsume: false,
  ...
  try {
    ...
    if (typeof entityOrUserId === "object" && entityOrUserId.entity_type) {
      if (entityOrUserId.entity_type === "USER") {
        ...
        console.error(
          "❌ [LICENSE SERVICE] COMPANY entity type is not valid for consumeFeature. Usage must be per-user.",
        );
        ...
        return {
          ...
          reason: "INVALID_ENTITY",
          message:
            "Feature consumption must be tracked per-user, not per-company.",
        };
      }
    } else {
      userId = entityOrUserId;
    ...
    const { start, end } =
      FeatureUsageTracking.getCurrentPeriod(legacyLimitType);
    const legacyQuery = {
      feature_code: feature_code,
      period_start: start,
    ...
    let legacyTracking =
      await FeatureUsageTracking.findOne(legacyQuery).session(session);
    ...
    if (!legacyTracking) {
      ...
      legacyTracking = await FeatureUsageTracking.create([createData], {
        session,
      });
    ...
    return {
      ...
      usage: {
        ...
        percentage:
          usageLimit === -1 || usageLimit === 0
            ? 0
            : Math.round((usageRecord.used_count / usageLimit) * 100),
 ...
 * @returns {Promise<Object>} { success: boolean, usage: Object }
 */
export const decrementFeature = async (
  entityOrUserId,
  feature_code,
  amount = 1,
) => {
  try {
    ...
    if (typeof entityOrUserId === "object" && entityOrUserId.entity_type) {
      if (entityOrUserId.entity_type === "USER") {
        ...
        console.error(
          "❌ [LICENSE SERVICE] COMPANY entity type is not valid for decrementFeature.",
        );
        return {
          success: false,
          reason: "INVALID_ENTITY",
          message:
            "Feature decrement must be tracked per-user, not per-company.",
        };
      }
    } else {
      userId = entityOrUserId;
    ...
    if (!usageRecord) {
      return {
        ...
        usage: {
          used: 0,
          limit: featureMapping?.usage_limit || -1,
          remaining: -1,
        },
      };
    }
    ...
    if (legacyTracking) {
      legacyTracking.usage_count = Math.max(
        0,
        legacyTracking.usage_count - amount,
      );
...
export const getLicenseSummary = async (entityOrUserId) => {
  try {
    ...
    return {
      ...
      trialExpired:
        userLicense.license_code === "EXPLORE" && userLicense.is_expired,
      entityType: "USER",
      entityId: userId,
      userId: userId,
...
export const getCompanyLicensePoolSummary = async (organizationId) => {
  try {
    ...
    const users = await User.find({ organization_id: organizationId })
      ...
      .select(
        "firstName lastName email role isPrimaryAdmin license_instance_id license_code assigned_license status",
      );
    ...
    console.error(
      "❌ [LICENSE SERVICE] Error in getCompanyLicensePoolSummary:",
      error,
    );
    throw error;
  }
...
export const getOrganizationLicenseInfo = async (organizationId) => {
  try {
    ...
    if (!subscription) {
      console.warn(
        `⚠️ [LICENSE SERVICE] No subscription found for organization ${organizationId}`,
      );
      return {
        organizationId: organizationId,
        subscription: null,
    ...
    if (!license) {
      console.warn(
        `⚠️ [LICENSE SERVICE] License not found for code ${subscription.license_code}`,
      );
      return {
        organizationId: organizationId,
        subscription: subscription,
    ...
    console.error(
      "❌ [LICENSE SERVICE] Error in getOrganizationLicenseInfo:",
      error,
    );
    throw error;
  }
 ...
 */
export const canAssignLicense = async (
  adminUserId,
  targetUserId,
  targetLicenseCode,
) => {
  try {
    ...
    if (!isSuperAdmin && !isPrimaryAdmin && !isOrgAdmin) {
      return {
        ...
        message:
          "Only Org Admin, Primary Admin, or Super Admin can assign licenses",
      };
    }
    ...
    if (isOrgAdmin && !isPrimaryAdmin && !isSuperAdmin) {
      ...
      if (adminUserId.toString() === targetUserId.toString()) {
        return {
          ...
          message:
            "You cannot assign a license to yourself. Contact the Primary Admin.",
        };
      }
...
export const assignLicenseToUser = async (
  ...
  try {
    ...
    const canAssign = await canAssignLicense(
      adminUserId,
      targetUserId,
      licenseCode,
    );
    ...
    const validation = await validateLicenseChange(
      targetUserId,
      currentLicenseCode,
      licenseCode,
    );
    ...
    if (targetUser.license_instance_id) {
      const currentInstance = await LicenseInstance.findById(
        targetUser.license_instance_id,
      ).session(session);
      if (currentInstance) {
    ...
    const licenseInstance = await LicenseInstance.findAvailableInstance(
      organizationId,
      licenseCode,
    );
  ...
  try {
    ...
    if (!isSuperAdmin && !isPrimaryAdmin && !isOrgAdmin) {
      ...
      return {
        ...
        reason: "NOT_AUTHORIZED",
        message:
          "Only Org Admin, Primary Admin, or Super Admin can unassign licenses",
      };
    }
    ...
    if (isOrgAdmin && !isPrimaryAdmin && !isSuperAdmin) {
      ...
      if (adminUserId.toString() === targetUserId.toString()) {
        ...
        return {
          ...
          message:
            "You cannot unassign your own license. Contact the Primary Admin.",
        };
      }
    ...
    return {
      ...
      reason: "UNASSIGNED",
      message:
        "License released back to pool. User now has EXPLORE (trial) license.",
    };
  } catch (error) {
    await session.abortTransaction();
    session.endSession();
    console.error(
      "❌ [LICENSE SERVICE] Error in unassignLicenseFromUser:",
      error,
    );
    throw error;
  }
...
export const checkDowngradeEligibility = async (userId, targetLicenseCode) => {
  try {
    ...
    const validationResult = await UserFeatureUsage.validateDowngrade(
      userId,
      targetMappings,
    );
    ...
    console.error(
      "❌ [LICENSE SERVICE] Error in checkDowngradeEligibility:",
      error,
    );
    throw error;
  }
 ...
 */
export const validateLicenseChange = async (
  userId,
  currentLicenseCode,
  targetLicenseCode,
) => {
  try {
    ...
    console.error(
      "❌ [LICENSE SERVICE] Error in validateLicenseChange:",
      error,
    );
    throw error;
  }
