...
class EmailToTaskService {
  constructor() {
    ...
    this.config = {
      user: (
        process.env.EMAIL_TO_TASK_EMAIL ||
        process.env.SMTP_USERNAME ||
        ""
      ).trim(),
      password: (
        process.env.EMAIL_TO_TASK_PASSWORD ||
  ...
  ensureUploadDir() {
    try {
      ...
      console.error(
        "📧 [EMAIL-TO-TASK] Failed to create upload directory:",
        error.message,
      );
    }
  }
  ...
  async processUnreadEmails() {
    ...
    const sinceDate = this.getImapDateString(
      this.serviceStartTime || new Date(),
    );
  ...
  async processAsComment(email, taskId) {
    try {
      const senderEmail = email.from?.value?.[0]?.address?.toLowerCase();
      const body = this.extractReplyContent(
      ...
      if (!task) {
        await this.sendRejectionEmail(senderEmail, "task_not_found", {
          taskId,
        });
      ...
      if (!hasAccess) {
        await this.sendRejectionEmail(senderEmail, "no_access", {
      ...
      console.error(
        "📧 [EMAIL-TO-TASK] Error processing comment:",
        error.message,
      );
  ...
  async createTaskFromEmail(email) {
    ...
    try {
      ...
      const senderEmail = email.from?.value?.[0]?.address?.toLowerCase();
      const senderName =
        email.from?.value?.[0]?.name || senderEmail?.split("@")[0];
      ...
      const isIndividualUser =
        user.role?.includes("individual") || user.accountType === "individual";
      ...
      try {
        const limitCheck = await licenseService.checkFeatureLimit(
          user._id,
          "TASK_EMAIL",
        );
        ...
        console.error(
          "📧 [STEP 4.5] ⚠️ Error checking TASK_EMAIL limit:",
          limitError.message,
        );
      ...
      const parsedData = this.parseEmailContent(
        rawSubject,
        body,
        user,
        organizationId,
      ...
      if (taskTitle.length > CONFIG.TASK_TITLE_MAX_LENGTH) {
        taskTitle =
          taskTitle.substring(0, CONFIG.TASK_TITLE_MAX_LENGTH - 3) + "...";
        warnings.push(
          `Title truncated to ${CONFIG.TASK_TITLE_MAX_LENGTH} characters`,
        );
      }
      ...
      const taskData = {
        ...
        description: parsedData.description.substring(
          0,
          CONFIG.TASK_DESCRIPTION_MAX_LENGTH,
        ),
        taskType: "regular",
      ...
      try {
        const usageResult = await licenseService.consumeFeature(
          user._id,
          "TASK_EMAIL",
          1,
        );
        ...
        console.error(
          "📧 [STEP 9] ⚠️ Error tracking TASK_EMAIL usage:",
          usageError.message,
        );
        // Don't fail task creation if usage tracking fails
      }
  ...
  parseEmailContent(subject, body, user, organizationId) {
    ...
    for (const pattern of priorityPatterns) {
      ...
      if (match) {
        ...
        if (PRIORITY_KEYWORDS[keyword]) {
          ...
          result.notes.push(
            `Priority auto-set to "${result.priority}" from keyword "${keyword}"`,
          );
    ...
    for (const pattern of dueDatePatterns) {
      ...
      if (match) {
        ...
        if (parsedDate) {
          ...
          result.notes.push(
            `Due date auto-set to "${TimezoneHelper.formatInTimezone(parsedDate, "UTC")}" from "${dueDateValue}"`,
          );
    ...
    if (!result.dueDate) {
      ...
      result.notes.push(
        `Due date auto-set to ${daysToAdd} day(s) from now based on "${result.priority}" priority`,
      );
    }
    ...
    for (const attachment of attachments) {
      ...
      if (processedCount >= CONFIG.MAX_ATTACHMENTS_COUNT) {
        result.warnings.push(
          `Maximum ${CONFIG.MAX_ATTACHMENTS_COUNT} attachments allowed - remaining attachments skipped`,
        );
        break;
      }
      ...
      if (
        ...
        CONFIG.TOTAL_ATTACHMENTS_MAX_SIZE_BYTES
      ) {
        result.warnings.push(
      ...
      try {
        ...
        result.warnings.push(
          `Failed to save attachment "${attachment.filename}": ${error.message}`,
        );
      }
    }
  ...
  async saveAttachment(attachment, userId) {
    ...
    try {
      ...
      console.error(
        "📧 [EMAIL-TO-TASK] Failed to save attachment:",
        error.message,
      );
      throw error;
    }
  ...
  async trackActivity(activityType, user, task, extraData = {}) {
    try {
      ...
      console.error(
        "📧 [EMAIL-TO-TASK] Failed to track activity:",
        error.message,
      );
    }
  }
  ...
  async sendRejectionEmail(email, reason, data = {}) {
    ...
    try {
      ...
      console.error(
        "📧 [EMAIL-TO-TASK] Failed to send rejection email:",
        error.message,
      );
    }
  }
  ...
  async sendConfirmationEmail(email, type, data = {}) {
    ...
    try {
      ...
      console.error(
        "📧 [EMAIL-TO-TASK] Failed to send confirmation email:",
        error.message,
      );
    }
  }
      ...
      imap.once("ready", () => {
        imap.openBox("INBOX", false, async (err, box) => {
          ...
          imap.search(
            ...
            async (searchErr, results) => {
              if (searchErr) {
                console.error(
                  "📧 [EMAIL-TO-TASK] ❌ Error searching emails:",
                  searchErr,
                );
                imap.end();
              ...
              fetch.on("message", (msg, seqno) => {
                msg.on("body", async (stream) => {
                  try {
                    ...
                    console.error(
                      "📧 [EMAIL-TO-TASK] ❌ Error parsing email:",
                      parseErr,
                    );
                    processed++;
                  }
