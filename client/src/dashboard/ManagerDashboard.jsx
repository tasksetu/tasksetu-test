...
const ManagerDashboard = () => {
    ...
    if (teamData?.employeeStats) {
      ...
      members = teamMembersRaw
        ...
        .map(m => {
          ...
          const activeTasks = mTasks.filter(t => t.status !== 'completed' && t.status !== 'DONE' && t.status !== 'COMPLETED').length;
          const overdueTasks = mTasks.filter(t =>
            (t.status !== 'completed' && t.status !== 'DONE' && t.status !== 'COMPLETED') && 
            t.dueDate && new Date(t.dueDate) < new Date()
          ).length;
          const completedTasks = mTasks.filter(t => t.status === 'completed' || t.status === 'DONE' || t.status === 'COMPLETED');
          ...
          return {
            ...
            lastActivity: m.lastLoginAt ? new Date(m.lastLoginAt).toLocaleDateString() : "Never"
          };
        });
    }
    
    console.log("Team members (org_admin filtered out, employees kept):", members.length);
    return Array.isArray(members) ? members : [];  
  }, [teamData, teamMembersRaw, teamTasksData]);

  const teamStatsForHeader = useMemo(() => {
    const totalTasks = teamTasksData.length;
    const completedTasks = teamTasksData.filter(t => t.status === 'completed' || t.status === 'DONE' || t.status === 'COMPLETED').length;
    const inProgressTasks = teamTasksData.filter(t => t.status === 'in_progress' || t.status === 'INPROGRESS').length;
    const overdueTasks = teamTasksData.filter(t =>
      t.dueDate && new Date(t.dueDate) < new Date() && 
      t.status !== 'completed' && t.status !== 'DONE' && t.status !== 'COMPLETED'
    ).length;
  ...
  const teamStats = useMemo(() => {
    ...
    return {
      ...
      teamProductivity: dashboardCounts.productivity || 
        (teamTasksData.length > 0 ? Math.round(((teamTasksData.length - teamStatsForHeader.overdueTasks) / teamTasksData.length) * 100) : 100),
      avgTaskCompletionTime: dashboardCounts.efficiency?.avgTaskCompletionTime || "N/A",
      upcomingDeadlines: dashboardCounts.beforeDueDateCount || 0,
      teamSatisfaction: dashboardCounts.productivity ? (dashboardCounts.productivity / 20).toFixed(1) : 4.8,
      isExpired: dashboardCounts.license?.isExpired || false
    };
  }, [dashboardCounts, teamMembers, teamData, teamTasksData, teamStatsForHeader]);
  ...
  const overdueFocusTasksData = useMemo(() => {
    ...
    return teamTasksData
      .filter(t => {
        ...
        return dueDate < now && status !== "DONE" && status !== "COMPLETED" && status !== "CANCELLED";
      })
      .sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate))
      .slice(0, 8);
  }, [teamTasksData]);
  ...
  const approvalFocusTasksData = useMemo(() => {
    return teamTasksData
      .filter(t => t.taskType === "approval" && t.status !== "DONE" && t.status !== "COMPLETED")
      .slice(0, 8);
  }, [teamTasksData]);
  ...
  }, [focusTab, todayFocusTasksData, overdueFocusTasksData, approvalFocusTasksData]);
  ...
  const kpiCards = useMemo(() => [
    ...
    },
    {
      ...
      onClick: () => setLocation(`/tasks?statusFilter=DONE&activeRole=${activeRole}`),
    },
    {
      label: "In Progress",
      ...
      onClick: () => setLocation(`/tasks?statusFilter=INPROGRESS&activeRole=${activeRole}`),
    },
    {
      label: "Overdue",
      value: teamStatsForHeader.overdueTasks,
      subtitle: teamStatsForHeader.overdueTasks > 0 ? `${teamStatsForHeader.overdueTasks} need attention` : "All on track",
      subtitleColor: teamStatsForHeader.overdueTasks > 0 ? "#dc2626" : "#16a34a",
      linkLabel: "View overdue",
      icon: AlertTriangle,
      iconColor: "text-red-600",
      iconBg: "bg-red-50",
      testId: "card-overdue",
      onClick: () => setLocation(`/tasks?dueDateFilter=overdue&activeRole=${activeRole}`),
    },
    {
      ...
      onClick: () => setLocation(`/reports?tab=team&activeRole=${activeRole}`),
      isProgressCard: true,
  ...
  ], [teamStatsForHeader, teamStats.teamProductivity, teamMembers, activeRole, setLocation]);
  ...
  const teamTasks = useMemo(() => {
    ...
    return teamTasksData.map(t => ({
      ...
      assignee: (() => {
        ...
        if (t.assignedTo?.firstName) return `${t.assignedTo.firstName} ${t.assignedTo.lastName || ""}`.trim();
  ...
  const filteredTasks = useMemo(() => {
    try {
      ...
      if (memberFilter !== "all") {
        tasks = tasks.filter(t => String(t.assigneeId || "") === String(memberFilter));
      }
      ...
      if (dueFrom) {
        tasks = tasks.filter(t => t.dueDate && new Date(t.dueDate) >= new Date(dueFrom));
      }
      if (dueTo) {
        tasks = tasks.filter(t => t.dueDate && new Date(t.dueDate) <= new Date(dueTo));
      }
      ...
      if (selectedTimeRange === "this_week") {
        ...
        tasks = tasks.filter(t => t.createdAt && new Date(t.createdAt) >= startOfMonth);
      } 
      else if (selectedTimeRange === "last_month") {
        const startOfLastMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1);
        const endOfLastMonth = new Date(now.getFullYear(), now.getMonth(), 0);
        tasks = tasks.filter(t => t.createdAt && new Date(t.createdAt) >= startOfLastMonth && new Date(t.createdAt) <= endOfLastMonth);
      }
      ...
      if (taskListFilter === "today") {
        ...
        tasks = tasks.filter(t => {
          ...
          dueDate.setHours(0, 0, 0, 0);
          const status = (t.status || "").toUpperCase();
          return dueDate < today && status !== "DONE" && status !== "COMPLETED" && status !== "CANCELLED";
        });
      }
  ...
  }, [teamTasks, searchQuery, memberFilter, priorityFilter, dueFrom, dueTo, selectedTimeRange, taskListFilter]);

  const projectStats = useMemo(() => {
    const milestones = milestonesData?.milestones || [];
    const taskBasedMilestones = teamTasksData.filter(t => t.isMilestone || t.taskType === 'milestone');

    const combined = [...milestones];

    taskBasedMilestones.forEach(tm => {
      if (!combined.some(m => m._id === tm._id || m.title === tm.title)) {
        const linkedIds = tm.linkedTasks?.length > 0 ? tm.linkedTasks : (tm.milestoneData?.linkedTaskIds || []);
    ...
    return combined.map(m => {
      ...
      if (m.isTaskBased) {
        const linkedTasks = teamTasksData.filter(t =>
          m.linkedTasks?.some(lt => (lt.taskId?._id || lt.taskId) === t._id)
        );
        if (totalTasks > 0 && !m.progressPercentage) {
          progress = Math.round((linkedTasks.filter(t => t.status === 'DONE').length / totalTasks) * 100);
        }
      }
  ...
  const efficiencyPie = useMemo(() => {
    ...
    for (const t of filteredTasks) {
      ...
      if (!isDone || !t.dueDate || !(t.completedDate || t.completedAt)) continue;
    ...
    if (onTime === 0 && late === 0) {
      ...
      return {
        ...
        series: [{
          type: 'pie',
          radius: ['45%', '70%'],
          data: [{ value: totalTasks, name: "Pending Tasks", itemStyle: { color: "#fbbf24" } }],
          label: { show: true, formatter: 'No completed tasks', position: 'center' }
        }]
      };
    }
  ...
  const workloadStacked = useMemo(() => {
    if (!teamMembers.length) {
      return {
        title: { show: true, text: 'No team members found', left: 'center', top: 'center' },
  ...
  const isLoading = membersLoading || tasksLoading || milestonesLoading || statsLoading;
  ...
  return (
    <div className="min-h-screen font-['Open_Sans','Helvetica_Neue',Helvetica,Arial,sans-serif]" style={{ backgroundColor: "#f3f3f4" }}>
      <div className="p-4">
        {/* PAGE HEADER */}
        <div className="bg-white border border-gray-200 rounded-sm px-5 py-4 shadow-sm mb-3 flex items-center justify-between flex-wrap gap-4">
          <div>
            <h2 className="text-[22px] font-medium m-0 leading-none" style={{ color: "#676a6c" }}>
              {(() => {
                const h = new Date().getHours();
                const greeting = h < 12 ? "🌤️ Good morning" : h < 17 ? "☀️ Good afternoon" : "🌙 Good evening";
                return (
                  <>
                    {greeting}, <span className="font-semibold text-blue-600">{user?.firstName || "Manager"}</span>
                  </>
                );
              })()}
            </h2>
            <p className="text-sm mt-2 m-0" style={{ color: "#9a9a9a" }}>
              Your team has <span className="font-semibold text-gray-700">{teamStatsForHeader.totalTasks}</span> tasks,{" "}
              <span className="font-semibold text-gray-700">{teamStatsForHeader.completedTasks}</span> completed.
            </p>
          </div>
          ...
          {(() => {
            ...
            const progressText = progressPercentage <= 20
              ? "Team needs a push 🚀"
            ...
            return (
              <div className="flex items-center gap-5">
                <div className="flex items-center gap-3 border-r border-gray-200 pr-5">
                  ...
                  <div>
                    <p className="text-xs text-gray-400 font-medium">Team progress</p>
                    <p className="text-sm font-semibold text-gray-700">
                      {progressPercentage >= 80 ? "Excellent" : progressPercentage >= 50 ? "On Track" : "Needs Focus"}
                    </p>
                  </div>
                </div>
                <div className="max-w-[180px]">
                  <p className="text-sm font-medium text-gray-700 leading-[18px]">{progressText}</p>
                </div>
              </div>
            );
          })()}
        ...
        <div className="bg-white border border-gray-200 shadow-sm rounded-sm mb-3">
          <div className="py-2.5 px-4 border-b border-gray-200 flex items-center justify-between rounded-sm" style={{ backgroundColor: "#f9f9f9" }}>
            <h5 className="text-xs font-semibold uppercase tracking-wider m-0 flex items-center gap-1.5" style={{ color: "#676a6c" }}>
              <Star size={12} className="text-yellow-500" /> Today Focus - Team Tasks
            </h5>
          ...
          {focusTabTasks.length > 0 ? (
            <div className="relative">
              <div className="overflow-x-auto scrollbar-thin" style={{ scrollbarWidth: "thin" }}>
                <div className="flex gap-2.5 p-3" style={{ minWidth: "max-content" }}>
                  {focusTabTasks.map((task) => (
                    <div
                      ...
                      onClick={() => setLocation(`/tasks/${task._id}?activeRole=${activeRole}`)}
                    >
                      <div className={`absolute left-0 top-0 h-full w-1 rounded-sm ${
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-start gap-2 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-gray-400 mt-0.5">Assigned to: {task.assignee || "Unassigned"}</p>
                          </div>
                        </div>
                      ...
                      <div className="mt-3 flex items-center justify-between">
                        <div>
                          <p className={`text-xs font-semibold ${
                            ...
                            {task.dueDate
                              ? (() => {
                                  const diff = Math.round((new Date(task.dueDate) - new Date()) / 60000);
                                  if (diff > 0 && diff < 1440) {
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-2.5 mb-3">
          {kpiCards.map(({ label, value, icon: Icon, iconColor, iconBg, testId, onClick, isProgressCard, percentage }) => (
            <div
              key={testId}
              onClick={onClick}
              data-testid={testId}
            ...
            >
              <div className="flex items-start justify-between mb-1">
                <div className="min-w-0">
                  <p className="text-[11px] font-semibold text-[#8e8e93] uppercase tracking-[0.08em] leading-none">{label}</p>
                  <h2 className="text-[30px] font-bold text-[#1f2937] tracking-tight leading-none mt-2">{value}</h2>
                </div>
                <div className={`w-10 h-10 flex items-center justify-center flex-shrink-0 shadow-sm border border-white/50 ${iconBg}`}>
              ...
              <div className="mt-auto pt-1 w-full opacity-95">
                {isProgressCard ? (
                  <svg viewBox="0 0 110 38" width="100%" height="38" preserveAspectRatio="none">
                    ...
                    <text x="0" y="33" fontSize="8" fill="#6b7280" fontFamily="sans-serif">Team productivity</text>
                  ...
                  <svg viewBox="0 0 110 38" width="100%" height="38" preserveAspectRatio="none">
                    <defs><linearGradient id="grad-completed" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <path d="M0,30 C10,26 20,24 30,19 C40,15 50,17 60,12 C70,8 80,10 90,5 C100,3 105,2 110,1 L110,38 L0,38 Z" fill="url(#grad-completed)" />
                  </svg>
                ) : label === "In Progress" ? (
                  <svg viewBox="0 0 110 38" width="100%" height="38" preserveAspectRatio="none">
                    <defs><linearGradient id="grad-progress" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <path d="M0,20 C8,11 16,28 24,18 C32,7 40,24 48,15 C56,8 64,21 72,12 C80,4 88,18 96,10 C102,5 106,8 110,6 L110,38 L0,38 Z" fill="url(#grad-progress)" />
                  </svg>
                ) : (
                  <svg viewBox="0 0 110 38" width="100%" height="38" preserveAspectRatio="none">
          ...
          <div className="bg-white rounded-sm shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
              ...
              <span className="text-xs text-gray-400 bg-gray-50 px-2 py-1 rounded-sm">
                {filteredTasks.filter(t => t.status === 'DONE' || t.status === 'COMPLETED').length} completed
              </span>
            </div>
            <div className="p-5">
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <div className="relative">
                        ...
                        <div className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-sm ring-2 ring-white ${getStatusColor(member.status)}`}></div>
                      </div>
                      <div>
                    ...
                    <div className="text-right">
                      <div className="flex items-center space-x-3 text-sm">
                        <div>
                          ...
                          <p className="font-semibold text-gray-700 text-xs">{member.activeTasks}</p>
                        </div>
                        <div>
                          <p className="text-rose-500 text-[10px]">Overdue</p>
                          <p className="font-semibold text-rose-600 text-xs">{member.overdueTasks}</p>
                        </div>
                        <div className="pl-2 border-l border-gray-200">
                          <p className="font-semibold text-gray-800 text-xs">{member.productivity}%</p>
                          <p className="text-[9px] text-gray-400">Productivity</p>
                        </div>
                      </div>
                    </div>
                  </button>
          ...
          <div className="space-y-3">
            <div className="bg-white rounded-sm shadow-sm border border-gray-100 flex flex-col">
              ...
              <div className="p-4 space-y-3">
                {projectStats.map((ms) => (
                  <div key={ms.name} className="border border-gray-100 rounded-sm p-3 bg-gray-50/30">
                    <div className="flex items-center justify-between mb-1.5">
                      <p className="text-sm font-semibold text-gray-800 truncate">{ms.name}</p>
                    ...
                    <div className="flex justify-between text-[11px]">
                      <span className="text-gray-500">{ms.totalTasks} tasks</span>
                      {ms.health === 'risk' ? (
                        <span className="text-rose-600 font-medium">⚠️ At risk</span>
                      ) : (
                ...
                <div className="space-y-2">
                  {weeklyPerformance.map((day, index) => (
                    <div key={index} className="flex items-center space-x-2">
                      <span className="text-xs font-medium text-gray-500 w-7">{day.day}</span>
                      <div className="flex-1 bg-gray-100 rounded-sm h-1.5">
                        <div
                          ...
                          style={{ width: `${Math.min(100, day.assigned > 0 ? (day.completed / day.assigned) * 100 : 0)}%` }}
                        />
                      </div>
                      <span className="text-xs text-gray-500">{day.completed}/{day.assigned}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {/* TEAM TASK LIST */}
        <div className="bg-white border border-gray-200 shadow-sm rounded-sm mt-3" id="team-tasks">
          <div className="py-2.5 px-4 border-b border-gray-200 rounded-sm" style={{ backgroundColor: "#f9f9f9" }}>
            <div className="flex items-center justify-between mb-2">
              <h5 className="text-xs font-semibold uppercase tracking-wider m-0" style={{ color: "#676a6c" }}>Team Task List</h5>
              <div className="flex gap-1">
                {[
                  { key: "all", label: "All", count: teamTasks.length },
                  { key: "today", label: "Today", count: teamTasks.filter(t => t.dueDate && new Date(t.dueDate).toDateString() === new Date().toDateString()).length },
                  { key: "overdue", label: "Overdue", count: teamTasks.filter(t => {
                    const now = new Date();
                    now.setHours(0, 0, 0, 0);
                    if (!t.dueDate) return false;
                    const dueDate = new Date(t.dueDate);
                  ...
                  <button
                    ...
                    style={taskListFilter !== tab.key ? { color: "#676a6c" } : {}}
                  >
                    {tab.label}{" "}
                    <span className={`ml-0.5 ${taskListFilter === tab.key ? "text-blue-200" : "text-gray-400"}`}>
                      {tab.count}
                    </span>
                  </button>
                ))}
            ...
            {filteredTasks.length > 0 ? (
              <table className="w-full">
                <thead style={{ backgroundColor: "#f9f9f9" }}>
                  <tr>
                    <th className="text-left py-2 px-3 text-xs font-semibold uppercase tracking-wide" style={{ color: "#676a6c" }}>Task</th>
                    <th className="text-left py-2 px-3 text-xs font-semibold uppercase tracking-wide" style={{ color: "#676a6c" }}>Assignee</th>
                    <th className="text-left py-2 px-3 text-xs font-semibold uppercase tracking-wide" style={{ color: "#676a6c" }}>Due Date</th>
                    <th className="text-left py-2 px-3 text-xs font-semibold uppercase tracking-wide" style={{ color: "#676a6c" }}>Priority</th>
                    <th className="text-left py-2 px-3 text-xs font-semibold uppercase tracking-wide" style={{ color: "#676a6c" }}>Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {filteredTasks.slice(0, 5).map((task) => (
                    <tr key={task.id} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => setLocation(`/tasks/${task.id}?activeRole=${activeRole}`)}>
                      <td className="py-2 px-3">
                        <div className="flex items-center gap-1.5">
                          {getTaskTypeIcon(task, 13)}
                          <span className="text-sm font-medium truncate max-w-[200px] block" style={{ color: "#676a6c" }} title={task.title}>
                            {task.title}
                          </span>
                        </div>
                      </td>
                      <td className="py-2 px-3 text-sm text-gray-600">{task.assignee}</td>
                      <td className="py-2 px-3 text-xs whitespace-nowrap" style={{ color: "#9a9a9a" }}>
                      ...
                      <td className="py-2 px-3">
                        <span className={`px-2 py-0.5 text-xs font-medium rounded-sm ${
                          task.priority === "high" ? "text-red-600 bg-red-50 border border-red-200" :
                          task.priority === "medium" ? "text-yellow-600 bg-yellow-50 border border-yellow-200" :
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-sm shadow-xl w-full max-w-md overflow-hidden">
            <div className="px-5 py-4 border-b flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-800">Schedule Report Email</h3>
              <Button variant="ghost" size="icon" onClick={() => setScheduleOpen(false)} className="rounded-sm">
                <X className="w-5 h-5" />
              </Button>
            </div>
            ...
            <div className="px-5 py-4 border-t flex justify-end gap-3 bg-gray-50">
              <Button variant="outline" className="h-9 rounded-sm border-gray-200" onClick={() => setScheduleOpen(false)}>Cancel</Button>
              <Button variant="default" className="h-9 rounded-sm bg-blue-600 hover:bg-blue-700" onClick={() => {
      ...
      {selectedMember && (
        <div className="fixed inset-0 z-50">
          <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={handleCloseMember} />
          <div className="absolute right-0 top-0 h-full w-full sm:w-[32rem] bg-white shadow-2xl border-l border-gray-100 flex flex-col animate-in slide-in-from-right duration-300">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-gray-50 to-white">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="font-semibold text-gray-800 text-lg">{selectedMember.name}</div>
                  <div className="text-xs text-gray-500">{selectedMember.role}</div>
                </div>
              </div>
              <Button variant="ghost" size="icon" onClick={handleCloseMember} className="rounded-sm">
            ...
            <div className="p-5 overflow-y-auto flex-1">
              ...
              <div className="space-y-4">
                {teamTasks.filter(t => String(t.assigneeId) === String(selectedMember.id)).map((t) => (
                  <div key={t.id} className="border border-gray-100 rounded-sm p-4 bg-white shadow-sm">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className={`inline-flex mt-2 px-2 py-0.5 rounded-sm text-xs font-medium ${getPriorityColor(t.priority)}`}>
                          Priority: {t.priority}
                        </div>
                      </div>
                    </div>
                ...
                {teamTasks.filter(t => String(t.assigneeId) === String(selectedMember.id)).length === 0 && (
                  <div className="text-center py-8">
                    <CheckSquare className="w-8 h-8 text-gray-300 mx-auto mb-2" />
                    <p className="text-sm text-gray-500">No tasks assigned to {selectedMember.name}.</p>
                  </div>
                )}
              </div>
            </div>
